...
- (void)reloadTableData
{
    ...
    [FLEXHeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id object, __unsafe_unretained Class actualClass) {
        NSUInteger count2 = (NSUInteger)CFDictionaryGetValue(mutableCountsForClasses, (__bridge const void *)actualClass);
        count2++;
        CFDictionarySetValue(mutableCountsForClasses, (__bridge const void *)actualClass, (const void *)count2);
    }];
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        NSUInteger count3 = (NSUInteger)CFDictionaryGetValue(mutableCountsForClasses, (__bridge const void *)(class));
        if (count3 > 0) {
            NSString *className = @(class_getName(class));
            [mutableCountsForClassNames setObject:@(count3) forKey:className];
        }
    }
    free(classes);
